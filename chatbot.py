...
{BOLD}Available commands:{RESET}
  {YELLOW}/joke{RESET}          — ask BojanBot for a joke
...
{GREEN}Hey Bojan! BojanBot here — specialising in code & tech.
...
""")
    ...
    while True:
        try:
            ...
            print(f"\n{CYAN}BojanBot:{RESET} Session ended. See you next time.\n")
            break
        ...
        if user_input.lower() == "quit":
            print(f"\n{CYAN}BojanBot:{RESET} Goodbye Bojan. 👋\n")
            break

        if user_input.lower() == "/clear":
            bot.reset()
            print(f"{CYAN}BojanBot:{RESET} {GRAY}Conversation cleared. Fresh start.{RESET}\n")
            continue

        if user_input.lower() == "/model":
            print(f"{CYAN}BojanBot:{RESET} Running {YELLOW}{bot.model}{RESET} via Groq.\n")
            continue
        ...
        if user_input.lower().startswith("/load "):
            ...
            if result is None:
                print(f"{CYAN}BojanBot:{RESET} {YELLOW}File not found:{RESET} {path}\n")
            else:
                print(f"{CYAN}BojanBot:{RESET} {GREEN}Loaded {result}.{RESET} Ask me anything about it.\n")
            continue

        elif user_input.lower() == "/loaded":
            if not bot.loaded_files:
                print(f"{CYAN}BojanBot:{RESET} {GRAY}No files loaded yet.{RESET}\n")
            else:
                files = "\n    ".join(bot.loaded_files)
                print(f"{CYAN}BojanBot:{RESET} Files in context:\n    {YELLOW}{files}{RESET}\n")
            continue
...
""")
        ...
        print(f"{GRAY}[Turn {bot.turn}]{RESET} {CYAN}{BOLD}BojanBot:{RESET} {reply}\n")
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n{CYAN}BojanBot:{RESET} Interrupted. Goodbye.\n")
